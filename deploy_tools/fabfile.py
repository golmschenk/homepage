...
REPO_URL = 'https://github.com/golmschenk/homepage.git'


def deploy():
    """
    Runs the automatic deployment process.
    """
...
def _create_directory_structure_if_necessary(site_folder):
    for subdirectory in ('database', 'static', 'virtualenv', 'source'):
        run('mkdir -p %s/%s' % (site_folder, subdirectory))
...
def _update_settings(source_folder, site_name):
    settings_path = source_folder + '/configuration/settings.py'
